...
- `fa/text` — core-блоки абзацу, заголовка, списку, цитати й розділювача, а також `fa/numbers`;
- `fa/numbers` — обгортка, яка приймає лише блоки `fa/number`;
- `fa/number` — елемент номера з `fa/title`, core-блоком зображення для іконки та `fa/text`;
